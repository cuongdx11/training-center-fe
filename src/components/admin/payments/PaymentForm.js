...
const PaymentForm = ({ payment, onClose, onUpdateStatus, mode = 'view' }) => {
            ...
            <div className="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-8">
                    ...
                    <div className="flex justify-end gap-4 pt-4 border-t">
                        ...
                        {mode === 'edit' && payment.status === 'PENDING' && (
                            <>
                                <button 
                                    onClick={() => onUpdateStatus(payment.id, true)}
                                    className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-200"
                                >
                                    Xác nhận thanh toán thành công
                                </button>
                                <button 
                                    onClick={() => onUpdateStatus(payment.id, false)}
